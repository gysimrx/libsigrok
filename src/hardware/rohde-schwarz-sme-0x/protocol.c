...
/*	[RS_CMD_PRESET] should be "*RST" and
	[RS_CMD_RESET_STATUS] should be "*CLS". But correcting them is not
	backward compatible with the previous implementation.
	Some apps/scripts may rely on the fact that the sme-0x driver only
	changes frequency/amplitude. */

	[RS_CMD_PRESET]           = NULL,
	[RS_CMD_RESET_STATUS]     = NULL,
	[RS_CMD_CONTROL_REMOTE]   = "\n",
